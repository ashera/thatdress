...
export async function dispatchEmailChangeRequest(
  ...
  const send = await sendEmail({
    to: newEmail,
    subject: "Confirm your new ebikeflip email",
    html: emailLayout({
      preheader: "Confirm the new email on your ebikeflip account.",
      heading: "Confirm your new email",
      body: `
        <p>You asked to change your ebikeflip login email from <strong>${escapeHtml(currentEmail)}</strong> to <strong>${escapeHtml(newEmail)}</strong>. Click the button below to make the switch — the link expires in 24 hours.</p>
